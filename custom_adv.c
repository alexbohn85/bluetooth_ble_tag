...
void fill_adv_packet(CustomAdv_t *pData, uint8_t flags, uint16_t companyID, uint8_t upperLFData, uint8_t lowerLFData, char *name)
{
	int n;
	...
	pData->upperLFData = upperLFData;
	pData->lowerLFData = lowerLFData;
...
void update_adv_data(CustomAdv_t *pData, uint8_t advertising_set_handle, uint8_t upperLFData, uint8_t lowerLFData)
{
  sl_status_t sc;
